...
describe('test CheckPermissions', () => {
  ...
  it('Correct Array permission authentication', () => {
    expect(checkPermissions(['user', 'admin'], target, error)('admin')).toEqual(
      'ok'
    )
  })
  it('Wrong Array permission authentication,currentAuthority error', () => {
    expect(
      checkPermissions(['user', 'admin'], target, error)('admin,user')
    ).toEqual('error')
  })
  it('Wrong Array permission authentication', () => {
    expect(checkPermissions(['user', 'admin'], target, error)('guest')).toEqual(
      'error'
    )
  })
  it('Wrong Function permission authentication', () => {
    expect(checkPermissions(() => false, target, error)('guest')).toEqual(
      'error'
    )
  })
