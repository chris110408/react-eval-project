...
const renderchildrenRender = (
  authority,
  childrenRender,
  noMatch,
  currentAuthority
) => childrenRender
const renderNoMatch = (authority, childrenRender, noMatch, currentAuthority) =>
  noMatch

const renderArrayAuth = (
  authority,
  childrenRender,
  noMatch,
  currentAuthority
...
export const createCurrentAuth = authority => {
  ...
  const getStringAuth = authority =>
    getTypeOf(authority) === 'String' ? authority : 'NULL'

  return ifElse(isFunction, authority => authority(), getStringAuth)(authority)
}

export const checkPermissions = (authority, childrenRender, noMatch) => (
  currentAuthority = null
) => {
  const _TypeOfAuth = getTypeOf(authority)
  const current = createCurrentAuth(currentAuthority)
  const check = ifElse(prop(_TypeOfAuth), prop(_TypeOfAuth), prop('Rest'))(
    CheckAuthFunctions
  )
